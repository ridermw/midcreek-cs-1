...
def git_fetch_traces(log: Path) -> list[str]:
    """Return every traced `git fetch`, including one with no arguments."""
    return [
        line
        for line in log.read_text(encoding="utf-8").splitlines()
        if re.search(r"(?:^|\s)(?:built-in:\s+)?git\s+fetch(?:\s|$)", line)
    ]
...
class DeepeningTest(unittest.TestCase):
    ...
    def test_the_trace_oracle_recognises_a_bare_fetch(self) -> None:
        log = self.root / "bare-fetch.log"
        log.write_text("trace: built-in: git fetch\n", encoding="utf-8")

        self.assertEqual(git_fetch_traces(log), ["trace: built-in: git fetch"])
    ...
    def test_a_short_clone_is_deepened_until_the_commit_is_reachable(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("every published commit is reachable", result.stdout)
        self.fetches = git_fetch_traces(log)
